...
# Hard-coded token for demonstration. In a real system, tokens would be issued by an auth server.
VALID_ACCESS_TOKEN = "secret-token"

# Decorator to enforce OAuth 2.0 access token validation
def require_oauth(f):
    @wraps(f)
...
<head>
    ...
    <style>
        ...
        a:hover { background-color: #357ab7; }
        .footer {
            text-align: center; margin-top: 20px;
            font-size: 0.9em; color: #777;
        }
...
# Protect the compute endpoint using our OAuth 2.0 decorator
